...
foreach ($brends as $brend) {  // get links for models
    ...
    foreach ($models as $model) {
        ...
        $response_rugs = $curl->get($model_item_url);
        $doc_rugs = phpQuery::newDocument($response_rugs->body);
        $rugs = $doc_rugs->find('.colors input');
        foreach ($rugs as $rug) {
            $rug_item = pq($rug);
            $rug_item_id = $rug_item->attr('id');
            $rug_item_color = $rug_item->attr('value');
            $rug_item_img_big = 'https://avtopilot1.ru' . $rug_item->attr('data-image');
            $rug_item_img_small = 'https://avtopilot1.ru' . $doc_rugs->find('.colors label[for='. $rug_item_id .'] img')->attr('src');

            $rug_item_img_big_extension = parse_url($rug_item_img_big, PHP_URL_PATH);
            $rug_item_img_big_extension = pathinfo($rug_item_img_big_extension, PATHINFO_EXTENSION);
            $rug_item_img_big_local_path = $model_path . '/' . 'big-' . $rug_item_color . '.' .$rug_item_img_big_extension;
            if( !file_exists($rug_item_img_big_local_path) ) {
                file_put_contents($rug_item_img_big_local_path, file_get_contents($rug_item_img_big));
            }

            $rug_item_img_small_extension = parse_url($rug_item_img_small, PHP_URL_PATH);
            $rug_item_img_small_extension = pathinfo($rug_item_img_small_extension, PATHINFO_EXTENSION);
            $rug_item_img_small_local_path = $model_path . '/' . 'small-' . $rug_item_color . '.' .$rug_item_img_small_extension;
            if( !file_exists($rug_item_img_small_local_path) ) {
                file_put_contents($rug_item_img_small_local_path, file_get_contents($rug_item_img_small));
            }
        }
    }
}
